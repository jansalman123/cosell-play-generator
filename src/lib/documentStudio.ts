...
export async function generateDocuments(input: DocumentGenerationInput): Promise<GeneratedDocuments> {
  ...
  try {
    const [executiveMarkdown, technicalMarkdown] = await Promise.all([
      withTimeout(executeGeminiRequest(buildDocumentPrompt(input, "executive"), false, { useSearch: true }), 30000, "Exec doc generation"),
      withTimeout(executeGeminiRequest(buildDocumentPrompt(input, "technical"), false, { useSearch: true }), 30000, "Tech doc generation")
    ]);
